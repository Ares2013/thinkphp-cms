 ...
 * @return string
 */
function xml_encode($data, $encoding='utf-8', $root='think',$itemname='item') {
    $xml    = '<?xml version="1.0" encoding="' . $encoding . '"?>';
    $xml   .= '<' . $root . '>';
    $xml   .= data_to_xml($data,$itemname);
    $xml   .= '</' . $root . '>';
    return $xml;
}
 ...
 * @return string
 */
function data_to_xml($data,$itemname='item') {
    $xml = '';
    foreach ($data as $key => $val) {
        is_numeric($key) && $key = $itemname." id=\"$key\"";
        $xml    .=  "<$key>";
