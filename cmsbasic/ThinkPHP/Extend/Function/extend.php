...
{
	$cstr = iconv('UTF-8', 'gbk', $str);
	$strLen = strlen($str);
	if(strlen($cstr) <= $length){
		return $str;
	}
	$count = 0;
	$newStr = '';
 ...
 * 把返回的数据集转换成Tree
 * @access public
 * @param array $list_old 要转换的数据集
 * @param string $pid parent标记字段
 * @param string $level level标记字段
 * @return array
 */
function list_to_tree($list_old, $pk='id',$pid = 'pid',$child = '_child',$root=0,$key_to_pk=false) {

	
    // 创建Tree
    $tree = array();
    if(is_array($list_old)) {
		$list=array();
		if($key_to_pk)
		{
