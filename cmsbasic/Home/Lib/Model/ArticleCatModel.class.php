	...
	public function article_cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0)
	{
		...
		if ($res === NULL)
		{
			...
			$sql = "SELECT c.*, COUNT(s.cat_id) AS has_children, COUNT(a.article_id) AS aricle_num ".
			   ...
			   " LEFT JOIN " . $this->getTableName() . " AS s ON s.parent_id=c.cat_id".
			   " LEFT JOIN `".$this->tablePrefix."article` AS a ON a.cat_id=c.cat_id".
			   " GROUP BY c.cat_id ".
		...
		else
		{
			foreach ($options AS $key => $value)
			{
				$options[$key]['url'] = U('article_cat', array('acid' => $value['cat_id']));
			}
			return $options;
		}
	}
